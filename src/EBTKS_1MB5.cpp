...
void loopTranslator(void)
{
    ...
    if (readCcr() & CCR_INT)
    {
        if (prevInt == false)
        {
            ...
            loadReadBuff(1, true);                   // 1 byte with PACK
        }
        prevInt = true;
    }
    else
...
void processOB(void)
{
    ...
    if (readOb(&ourOB))
    {
        ...
        else
        {
            ...
            {
            case 0: //read status
                readBuff[0] = SR[prevCmd & 0x0f];
                loadReadBuff(1);
                LOGPRINTF_1MB5(" repeat read SR#%02d Val:%02X", prevCmd & 0x0f, SR[prevCmd & 0x0f]);
                prevCmd++;
                prevCmd &= 0x0fu;
                break;
